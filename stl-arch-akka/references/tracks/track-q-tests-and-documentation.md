...
- [ ] Repo-root `AGENTS.md` mirrors onion rules (see skill `SKILL.md` → Repo documentation) and points coding style at `stl-convention`; covers layers, remap, serialization (two ObjectMappers + `json-serialization-template`), `impl.enums`, tags/logs, setup/rebuild, audits when applicable.
